...
function handleLoginRequest(req, res, next) {
	...
	var sql = "SELECT * FROM User U WHERE U.username=$1;";
	db.query(sql, [username], function (e1, d1) { 
		handleLoginRequest1(req, res, next, e1, d1); 
	});
}
...
function handleLoginRequest1(req, res, next, err, data) {
	...
	if (err) {
		...
		return res.render("login", { userName: username, password: "", loginError: invalidUserNameErrorMessage });
	} else {
		var userdata = data.rows[0];
		if (!comparePassword(password,userdata.password)) {
			return res.render("login", { userName: username, password: "",	loginError: invalidPasswordErrorMessage } );
		}
...
function handleSignup(req, res, next) {
	...
	if (validateSignup(userName, firstName, lastName, password, verify, email, errors)) {
		...
		var sql = "SELECT * FROM User U WHERE U.userName=$1;";
		db.query(sql, [userName], function (e1, d1) { 
			handleSignup1(req, res, next, errors, e1, d1);
		});
	} else {
...
function handleSignup1(req, res, next, errors, err, data) {
	...
	var sql = "INSERT INTO User(userName, firstName, lastName, password, email) VALUES($1, $2, $3, $4, $5);";

	db.query(sql, [userName, firstName, lastName, password, email], function(e1, d1) { 
		handleSignup2(req, res, next, e1, d1); 
	});
}
...
function handleSignup2(req, res, next, err, data) {
	...
	// " '" + userName + "'";
	db.query(sql, [userName], function (e1, d1) { 
		handleSignup3(req, res, next, e1, d1); 
	});
}
...
function handleSignup3(req, res, next, err, data) {
	...
	req.session.regenerate(function () { 
		handleSignup4(req, res, next, user); 
	});
}
...
function prepareUserData(user, next) {
	...
	//var sql = "INSERT INTO Allocations (userId,stocks,funds,bonds) VALUES ( " +
	//  	user.userId + "," + stocks + "," + funds + "," + bonds + ")";

	var sql = "INSERT INTO Allocations(userId, stocks, funds, bonds) VALUES($1, $2, $3, $4);";
	db.query(sql, [userId, stocks, funds, bonds], function (e1, d1) { 
		prepareUserData1(user, next, e1, d1); 
	});
}
...
function prepareUserData1(user, next, err, data) {
	...
	var sql = "INSERT INTO Profile(userid) VALUES($1);";
	db.query(sql, [userId], function(e1, d1) { 
		prepareUserData2(user, next, e1, d1); 
	});
}
...
function prepareUserData2(user, next, err, data) {
	...
	var sql = "INSERT INTO Contribututions(userid) VALUES($1);";
	db.query(sql, [userId], function(e1, d1) { 
		prepareUserData3(user, next, e1, d1); 
	});
}
...
function displayWelcomePage(req, res, next) {
	...
	var sql = "SELECT * FROM User WHERE userId=$1;";// + userId;
	db.query(sql, [userId], function (e1, d1) { 
		displayWelcomePage1(req, res, next, e1, d1); 
	});
}
